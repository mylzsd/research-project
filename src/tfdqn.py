...
def ann(input_, hiddens, num_act):
    ...
    for hidden in hiddens:
        ...
        # out = layers.fully_connected(out, num_outputs=hidden, activation_fn=tf.compat.v1.nn.tanh)
        out = layers.fully_connected(out, num_outputs=hidden, activation_fn=tf.compat.v1.nn.relu)
        # out = tf.compat.v1.nn.leaky_relu(out, 0.8)
...
class DQN:
    def __init__(self, env, hiddens, learning_rate, discount_factor):
        ...
        self.input_state = tf.compat.v1.placeholder(np.float64, shape=(None, input_size))
        # compose deep Q network
        self.network = ann(self.input_state, hiddens, self.num_act)
        self.target = tf.compat.v1.placeholder(np.float64, None)
        self.loss = tf.compat.v1.losses.mean_squared_error(labels=self.target, predictions=self.network)
    ...
    def policy(self, state, randomness=0.0):
        ...
        # exploitation
        batch_in = np.reshape(state.getPred(one_hot=True), (1, -1))
        q_values = self.sess.run(self.network, feed_dict={self.input_state: batch_in})
        act_val = Counter()
        for a in actions:
    ...
    def train(self, state, action, state_p, reward, in_set, in_row):
        ...
        batch_in = np.reshape(state.getPred(one_hot=True), (1, -1))
        feed_dict = {self.input_state: batch_in}
